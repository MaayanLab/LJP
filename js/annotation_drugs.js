function render_graph(graph) {
	...
	function getParamsFromControler() {
		var params = {};
		var keys = ['Drug', 'direction', 'library'];
	...
	d3.json(config['ENTER_POINT'] + '/controls?table_name=Drug&', function (controlAttrs) {
			...
			function drawTable(params) {
                ...
                $('#controls').append(
                	'<div id="enrichment-table-wrapper">' +
	                    ...
	                    '<div id="table"></div>' +
	                    '<a id="downloadTable" href="data/drugname_enrichment_table.csv" download>Download the full enrichment table</a>' +
                    '</div>'
                );
				...
				d3.json(config['ENTER_POINT'] + '/controls?table_name=Drug&' + getParams, function (tableData) {
